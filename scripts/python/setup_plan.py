...
def _help_text(argv0: str) -> str:
    return f"""Usage: {argv0} [--json] [--paths-only]
  --json        Output results in JSON format
  --paths-only  Resolve paths without writing files
  --help        Show this help message
"""


def _emit_results(paths: FeaturePaths, json_mode: bool) -> None:
    if json_mode:
        sys.stdout.write(
            _json_line(
                {
